...
```bash
tmp="$(mktemp -d)"
git clone --depth 1 https://github.com/Vo1ganin/dune-skill.git "$tmp/repo"
rm -rf "$HOME/.claude/skills/dune"
mkdir -p "$HOME/.claude/skills"
cp -R "$tmp/repo" "$HOME/.claude/skills/dune"
rm -rf "$HOME/.claude/skills/dune/.git" "$tmp"
```
